...
DEFAULT_TICKERS = DATA_CONFIG.get("default_tickers", ["AAPL"])
DEFAULT_TICKER_CIK = DATA_CONFIG.get("sec_default_ticker_cik", {})
FIELDS = DATA_CONFIG["sec_fields"]
ALLOWED_FORMS_YEARLY = set(DATA_CONFIG["sec_allowed_forms"])
ALLOWED_FORMS_QUARTERLY = set(DATA_CONFIG.get("sec_allowed_forms_quarterly", []))
DEFAULT_YEARS = int(DATA_CONFIG.get("sec_years_default", 10))
DEFAULT_SEC_DIR_YEARLY = DATA_CONFIG.get("sec_data_dir", "data/sec/yearly")
DEFAULT_SEC_DIR_QUARTERLY = DATA_CONFIG.get("sec_data_dir_quarterly", "data/sec/quarterly")
    ...
    for ticker in tickers:
        ...
        if args.freq in ("quarterly", "both"):
            if not ALLOWED_FORMS_QUARTERLY:
                raise RuntimeError("Quarterly forms list is empty. Update data_config.json.")
